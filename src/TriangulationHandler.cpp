...
void TriangulationHandler::init_triangulation(std::vector<int>& triangles) {
...
void TriangulationHandler::init_triangulation(std::vector<int16_t> &triangles) {
    std::vector<Edge> edge_representation;
    if(!std::is_sorted(triangles.begin(),triangles.end())){
        std::sort(triangles.begin(),triangles.end());
    }
...
Edge TriangulationHandler::do_flip_for_real(Edge e) {
    ...
    for(int i=0;i<_triangle_representation.size();i++){
        if(_short_representation[i]==ind_o1){
            _short_representation[i]=(short)ind_n1;
            _triangle_representation[i]=new_1;
        }
        if(_short_representation[i]==ind_o2){
            _short_representation[i]=(short)ind_n2;
...
std::vector<int16_t> TriangulationHandler::get_representation() {
    std::vector<int16_t> result(_short_representation);
    return result;
}
...
//    for(int i=0;i<target_triangulation._short_representation.size();i++){
//        if(_short_representation[i]!=target_triangulation._short_representation[i]){
//            return false;
//        }
//    }
//    return true;
...
//    Triangle new_2{f_0,f_1,e_1};
    ...
    _short_representation[pos]=(short)ind_n1;
    bubble_to_correct_spot(_short_representation,pos);

    pos= binarySearch(_short_representation,ind_o2);
    _short_representation[pos]=(short)ind_n2;
    bubble_to_correct_spot(_short_representation,pos);

    /*for(int i=0;i<_short_representation.size();i++){
        if(_short_representation[i]==ind_o1){
            _short_representation[i]=(short)ind_n1;
        }
        if(_short_representation[i]==ind_o2){
...
int TriangulationHandler::binarySearch(const std::vector<int16_t> &vec, int val) {
    int low = 0;
    int high = vec.size() - 1;
...
void TriangulationHandler::bubble_to_correct_spot(std::vector<int16_t> &vec, int pos) {
    int nn = vec.size();
    bool isSwapped = true;
...
std::pair<std::vector<int16_t>,int> TriangulationHandler::do_flip_implicit_for_simple_heuristic(Edge e,const StaticTriangulation& target){
    int offset=0;
...
int TriangulationHandler::get_eppstein_heuristic_parent(const StaticTriangulation &target_triangulation) {
    ...
    //std::cout<<result<< " parent"<<std::endl;
    return result;
}
...
int TriangulationHandler::get_eppstein_heuristic_child(const StaticTriangulation &target_triangulation,
    ...
    //std::cout<<result<< " child"<<std::endl;
    return result;
}
...
TriangulationHandler::get_relevant_edges_for_eppstein(const StaticTriangulation &target_triangulation) {
    ...
    while(start_index<target_triangulation._interior_edge_representation.size()&&target_index<target_triangulation._interior_edge_representation.size()){
        if(_interior_edge_representation[start_index]<target_triangulation._interior_edge_representation[target_index]){
            start.emplace_back(_interior_edge_representation[start_index]);
            start_index++;
        } else if((_interior_edge_representation[start_index]>target_triangulation._interior_edge_representation[target_index])){
            target.emplace_back(target_triangulation._interior_edge_representation[target_index]);
            target_index++;
        }
        else{
            start_index++;
        ...
        while(start_index<_interior_edge_representation.size()){
            start.emplace_back(_interior_edge_representation[start_index]);
            start_index++;
        }
        while(target_index<target_triangulation._interior_edge_representation.size()){
            target.emplace_back(target_triangulation._interior_edge_representation[target_index]);
            target_index++;
        }
    }
    ...
    /*for(int i=0;i<_edge_matrix.size();i++){
        for(int j=0;j<_edge_matrix[0].size();j++){
           if(_edge_matrix[i][j]>=0&&target_triangulation._edge_matrix[i][j]<0 ){
               start.emplace_back(_objects.get_edge(i,j));
           }
            if(_edge_matrix[i][j]<0&&target_triangulation._edge_matrix[i][j]>=0 ){
                target.emplace_back(_objects.get_edge(i,j));
            }
        }
    }*/
    return {start,target};
...
TriangulationHandler::get_distance_matrix_for_eppstein(const StaticTriangulation &target_triangulation) {
    ...
    std::vector<std::vector<int>> matrix(x.first.size(), std::vector<int>(x.first.size(), 0));
...
std::vector<Triangle> TriangulationHandler::do_n_valid_flips(int n, int seed) {
    ...
    while(counter<n){
        ...
        /*if(std::find(already_considered.begin(),already_considered.end(),_interior_edge_representation[ra])!=already_considered.end()){
            std::cout<<"happend";
            continue;
        }*/
