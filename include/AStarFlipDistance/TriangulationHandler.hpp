...
public:
    ...
    void init_triangulation(std::vector<int>& triangles);
    void init_triangulation(std::vector<int16_t>& triangles);
    ...
    std::pair<std::vector<int16_t>,int> do_flip_implicit_for_simple_heuristic(Edge e,const StaticTriangulation& target);
    ...
    std::vector<std::vector<int>> get_distance_matrix_for_eppstein(const StaticTriangulation & target_triangulation);
    ...
    std::vector<int16_t> get_representation();
...
private:
    ...
    std::vector<int16_t> _short_representation;

    int binarySearch(const std::vector<int16_t>& vec, int val);
    void bubble_to_correct_spot(std::vector<int16_t>& vec,int pos);
